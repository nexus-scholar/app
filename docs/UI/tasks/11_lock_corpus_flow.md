...
2. **Post-lock Banner Machine (Overview page):**
   ...
   - Replaced by a permanent `LifecycleStatusBar` chip upon dismissal or auto-hide.
      ...
      - Use the `dialog.tsx` for the Lock modal and the `banner`/`chip` components for the post-lock states. Persist banner dismissal state in localStorage (with expiry) or via server if preferred.
      - Keep time-based rules implemented as a small utility function to centralise calculations and make unit testing straightforward.
