...
- Frontend stack: Inertia v3 (`@inertiajs/react` v3), React v19, Tailwind CSS v4, Vite, TypeScript. Ensure any UI notes assume these versions.
- UI components: This repo uses the shadcn-style component structure. Reusable UI components live under `resources/js/components/ui` and `components.json` config is present at the repo root. Prefer reusing those components and follow the existing `cn` + `cva` patterns.
- Routes & actions: Use Wayfinder-generated helpers when wiring frontend-to-backend routes (import from `@/routes` or `@/actions`).
- HTTP: Inertia v3 ships a built-in XHR client; prefer `useHttp` or Inertia visits for requests. Do not add Axios unless there's a compelling reason.
- Icons: `lucide-react` is the icon library in use.
