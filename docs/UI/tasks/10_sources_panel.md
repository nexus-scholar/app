...
2. **Expanded State:** 
   ...
   - Show tie-break rule tooltip if multiple rows have the same score.
    ...
    - Reuse the existing `collapsible.tsx`, `chip.tsx`, and table patterns in `resources/js/components/ui` for panel and table rendering.
    - Use semantic HTML tables and ensure keyboard focus, `aria-sort`/`aria-label` as applicable for accessibility.
