...
- Reuse `input.tsx`, `card.tsx`, and `table` patterns for API key manager and audit log. Role-based UI should be driven from Inertia props and centralised in a permissions context/provider.
- The notification dropdown should reuse `dropdown-menu.tsx` and `sonner` for toast notifications which is already present in dependencies.
