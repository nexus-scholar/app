...
3. **Polling Logic:** 
   ...
   - Freezes polling and timestamp when the run reaches a terminal state.
      ...
      - Reuse the existing `RunStatusCard` and `spinner/skeleton` components under `resources/js/components/ui` where possible.
      - Implement polling using a React hook that uses `setInterval` and clears on unmount; if using Inertia visits prefer `router.get()` with `preserveState` or `useHttp` for ad-hoc requests.
      - Keep polling interval logic in a single hook that can be reused across the app to avoid duplicate timers.
