...
- Use the existing `sheet.tsx` (drawer) component located in `resources/js/components/ui/sheet.tsx` for `WorkDetailDrawer` and `tooltip.tsx` for tooltips.
- Implement the micro-grid and identity chips by reusing `chip.tsx`/`badge.tsx` components and `cva` variants to reflect presence/absence.
- Use `lucide-react` via the repo's `icon.tsx` wrapper to ensure consistent icon sizing and accessibility.
