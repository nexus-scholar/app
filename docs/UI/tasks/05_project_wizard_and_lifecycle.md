...
- Reuse shadcn-style `dialog.tsx` for the wizard and `chip`/`badge` components for step indicators. Keep the wizard as a controlled component that can be rendered inside the existing modal wrapper.
- Use Tailwind v4 utility classes and `cva` for stepper and chip variants to keep consistent styling and accessible focus states.
