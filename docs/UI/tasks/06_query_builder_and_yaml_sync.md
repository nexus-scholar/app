...
- The UI should live under `resources/js/pages` or `resources/js/components` and reuse existing input and textarea components (see `ui/input.tsx` and `ui/textarea.tsx`).
- Prefer using a lightweight YAML editor component (e.g., CodeMirror) if needed; keep it optional and ensure it plays nicely with Tailwind v4 styling.
- Use Inertia's `useHttp` or Inertia form submissions for any save actions. Do not add Axios by default.
