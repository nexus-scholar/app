...
- Use an existing stepper/step components (or create a small `stepper.tsx` in `resources/js/components/ui`) and use `cva` variants for step states.
- For live previews of BibTeX/RIS/JSON, use a lightweight code viewer such as `react-simple-code-editor` or CodeMirror with Tailwind-friendly styles.
