...
- Implement keyboard shortcuts via a small custom hook (e.g., `useHotkeys`) and centralise bindings so they can be disabled in inputs. Reuse existing `chip.tsx`, `collapsible.tsx`, and `dialog.tsx` components.
- Keep desktop layout styles in Tailwind v4 and ensure focus management for keyboard flows.
