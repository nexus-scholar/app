...
- Prefer a small gesture helper like `use-gesture` or `@use-gesture/react` (verify compatibility with React v19) and ensure it only depends on well-maintained packages. Keep the gesture handler encapsulated in a hook that can be reused.
- Ensure horizontal swipes are disabled at the app level to avoid triggering browser history. Provide clear onboarding via an existing `sheet.tsx` component.
