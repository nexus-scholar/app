...
- Use existing `sidebar.tsx`, `chip.tsx`, and `card.tsx` components under `resources/js/components/ui` for consistent behavior.
- Ensure responsive behavior uses Tailwind v4 breakpoints and that sidebar toggles are realised via the existing `SidebarProvider`/`AppShell` pattern.
